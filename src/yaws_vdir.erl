...
arg_rewrite(ARG) ->
	...
	{abs_path, RawPath} ->
		...
		{"", _QueryPart} ->
		    ARG2 = ARG;
		{"/", _QueryPart} ->
		    %%don't allow vdir to be specified for root - it doesn't make sense
		    ARG2 = ARG;
		{DecPath, _QueryPart} ->
		    SC = get(sc),
		    
		    %%vdirpath/3 will return the longest(ie most specific) 'virtual directory' match for our request
			...
			{Virt,DocRoot} ->
			    ...
			    %%the virtual-path of our request matches a vdir specification 
			    %% - rewrite ARG accordingly.

			    ARG2 = ARG#arg{docroot = DocRoot,docroot_mount = Virt}
		    end
	    end;
	_Else ->
	    ARG2 = ARG
