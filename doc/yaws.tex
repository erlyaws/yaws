...
\item        \verb+is_default = true|false+ -
              Sets the virtual host defined in this server part to be
              the default.  This means that if there are several
              virtual hosts defined for the same IP number and port,
              and an HTTP request arrives with a Host field that does
              not match any defined virtual host, then the one with
              \verb+is_default+ set to true is chosen. Do not set
              \verb+is_default+ to true in more than one server part.
